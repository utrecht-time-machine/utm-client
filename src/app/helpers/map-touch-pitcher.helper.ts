...
/**
 *  Class that enables mobile gesture handling for pitch.
 *  Provided by https://github.com/HarelM in https://github.com/mapbox/mapbox-gl-js/issues/3405#issuecomment-501982132
 */
export class MapTouchPitcherHelper {
  /**
  ...
  private handleTouchStart(touchEvent: MapTouchEvent) {
    ...
    if (
      Math.abs(diffX) < MapTouchPitcherHelper.MIN_DIFF_X
      || Math.abs(diffY) > MapTouchPitcherHelper.MAX_DIFF_Y
      || Math.abs(diffX) > MapTouchPitcherHelper.MAX_DIFF_X
    ) {
      return;
    }
  ...
  private handleTouchMove(touchEvent: MapTouchEvent) {
    ...
    const isTimePassed =
      Date.now() - this.startTiming >= MapTouchPitcherHelper.DELAY;
    if (
      Math.abs(distance - this.startDistance) >= MapTouchPitcherHelper.MIN_DIFF
    ) {
      const eventData = isTimePassed
        ? touchEvent.originalEvent
        : this.startEventData.originalEvent;
      this.resetInteractions();
